...
// Reassign static lines to use the faint static gradient. They were originally "1.5" stroke width.
svg = svg.replace(/stroke="url\(#well-gradient\)"/g, 'stroke="url(#well-gradient-static)"');
...
// Update the thick animated paths (strokeWidth="2") to be highly visible and use the vibrant animated gradient
svg = svg.replace(/<path strokeWidth="2" d="([^"]+)"[^>]*>/g,
...
// Update the thin animated paths (strokeWidth="1")
svg = svg.replace(/<path strokeWidth="1" d="([^"]+)"[^>]*>/g,
...
console.log('SVG Rebuilt successfully. It will now be visible.');
